...
class GPT():
    def __init__(self, dim, head_size, vocab_size, X, Y):
        ...
        # X-head
        self.WxQ, self.WxK, self.WxV = np.random.randn(3, self.head_size, self.inner_dim, self.inner_dim / self.head_size)
        # Y-head
        self.WyQ, self.WyK, self.WyV = np.random.randn(3, self.head_size, self.inner_dim, self.inner_dim / self.head_size)
    ...
    def forward_Y(self, Y_input, X_output, embed, inner_dim, WQ, WK, WV, WZ, MHA_alpha, MHA_beta, Wf1, Wf2, Bf1, Bf2, Of_alpha, Of_beta):
    ...
    def full_forward(self):
        ...
        Y_output = self.forward_Y(Y_input=self.Y, X_output=X_output, inner_dim=self.inner_dim, WQ=self.WyQ, WK=self.WyK, WV=self.WyV, WZ=self.WyZ, MHA_alpha=self.mha_cross_alpha, MHA_beta=self.mha_cross_beta)
